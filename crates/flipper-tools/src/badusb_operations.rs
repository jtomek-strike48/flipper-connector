...
// ============================================================================
// BadKB (Bluetooth Keyboard) Tool
// ============================================================================

pub struct BadKbUploadTool;

#[async_trait]
impl PentestTool for BadKbUploadTool {
    fn name(&self) -> &str {
        "flipper_badkb_upload"
    }

    fn description(&self) -> &str {
        "Upload a BadKB script for Bluetooth keyboard emulation (Unleashed firmware)"
    }
        ...
        ToolSchema {
            ...
            params: vec![
                ...
                ToolParam {
                    name: "script".to_string(),
                    param_type: ParamType::String,
                    description: "Ducky Script content (same format as BadUSB)".to_string(),
                    required: true,
                    default: None,
                },
                ToolParam {
                ...
                ToolParam {
                    name: "bt_name".to_string(),
                    param_type: ParamType::String,
                    description: "Bluetooth device name (optional, for identification)".to_string(),
                    required: false,
                    default: None,
                },
            ],
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> flipper_core::error::Result<ToolResult> {
        ...
        let validate = params["validate"].as_bool().unwrap_or(true);
        let bt_name = params["bt_name"].as_str();
        ...
        // Create BadKB script with header comments
        let mut full_script = String::new();
        full_script.push_str("REM BadKB (Bluetooth Keyboard) Script\n");
        full_script.push_str("REM Generated by Flipper Zero Connector\n");
        if let Some(name) = bt_name {
            full_script.push_str(&format!("REM BT Device: {}\n", name));
        }
        full_script.push_str("REM Mode: Bluetooth HID\n");
        full_script.push_str("REM To use: Open BadUSB app, select script, choose BT mode\n");
        ...
        // Upload script
        client.write_file(&path, full_script.as_bytes().to_vec()).await
            .map_err(|e| flipper_core::error::Error::ToolExecution(e.to_string()))?;

        let line_count = script.lines().count();
        let script_size = full_script.len();
        ...
        Ok(ToolResult {
            ...
            data: json!({
                ...
                "size": script_size,
                "size_formatted": format_size(script_size as u32),
                "mode": "bluetooth",
                "bt_name": bt_name,
                "usage": "Open BadUSB app on Flipper Zero, select script, and choose Bluetooth mode to execute wirelessly"
            }),
            error: None,
            duration_ms: 0,
        })
...
pub struct BadKbExecuteTool;

#[async_trait]
impl PentestTool for BadKbExecuteTool {
    fn name(&self) -> &str {
        "flipper_badkb_execute"
    }

    fn description(&self) -> &str {
        "Execute a BadKB script via Bluetooth (inline script, no file upload)"
    }
        ...
        ToolSchema {
            ...
            params: vec![
                ToolParam {
                    name: "script".to_string(),
                    param_type: ParamType::String,
                    description: "Ducky Script to execute immediately".to_string(),
                    required: true,
                    default: None,
                },
                ToolParam {
                    name: "validate".to_string(),
                    param_type: ParamType::Boolean,
                    description: "Validate script before execution".to_string(),
                    required: false,
                    default: Some(json!(true)),
                },
                ToolParam {
                    ...
                    param_type: ParamType::Boolean,
                    description: "Delete temporary script file after execution".to_string(),
                    required: false,
                    default: Some(json!(true)),
                },
            ],
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> flipper_core::error::Result<ToolResult> {
        ...
        let validate = params["validate"].as_bool().unwrap_or(true);
        let cleanup = params["cleanup"].as_bool().unwrap_or(true);
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        let temp_filename = format!("badkb_temp_{}.txt", timestamp);
        let path = format!("{}/{}", BADUSB_DIR, temp_filename);

        // Prepare script with header
        let mut full_script = String::new();
        full_script.push_str("REM BadKB Inline Execution\n");
        full_script.push_str("REM Auto-generated temporary script\n\n");
        full_script.push_str(script);

        let mut client = FlipperClient::new()
            .map_err(|e| flipper_core::error::Error::ToolExecution(e.to_string()))?;

        // Upload temporary script
        client.write_file(&path, full_script.as_bytes().to_vec()).await
            .map_err(|e| flipper_core::error::Error::ToolExecution(e.to_string()))?;
        ...
        let result = json!({
            ...
            "size": full_script.len(),
            "mode": "bluetooth",
            "status": "uploaded",
            "note": "Open BadUSB app and select script to execute in Bluetooth mode"
        });

        // Cleanup if requested (note: in real implementation, this would happen after execution)
        if cleanup {
            // For now, we just note that cleanup is requested
            // In a real implementation with app control API, we'd wait for execution and then delete
        }
